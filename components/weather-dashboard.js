...
class WeatherDashboard {
  ...
  updateUI(data) {
    ...
    if (temp !== undefined && temp !== null) {
      outdoorCombined.querySelector(".temp-value").textContent =
        `${parseFloat(temp).toFixed(1)}°F`;
    ...
    if (humidity !== undefined && humidity !== null) {
      outdoorCombined.querySelector(".humidity-value").textContent =
    ...
    if (windSpeed !== undefined && windSpeed !== null) {
      windCard.querySelector(".wind-speed-value").textContent =
        `${parseFloat(windSpeed).toFixed(1)} mph`;
      this.createWindAnimation(windCard, parseFloat(windSpeed), windDir);
    }

    if (windGust !== undefined && windGust !== null) {
      windCard.querySelector(".gust").textContent =
        `Gust: ${parseFloat(windGust).toFixed(1)} mph`;
    }
    ...
    if (indoorCard) {
      ...
      this.addIndoorAnimation(
        "#indoor-sensors",
        parseFloat(indoorTemp),
        parseFloat(indoorHum),
      );
    }
    ...
    const progress =
      ((now.getTime() % this.updateInterval) / this.updateInterval) * 100;
  ...
  createSolarAnimation(container, uvIndex, solarValue) {
    ...
    const solarSize = !isNaN(solarValue)
      ? Math.min(65, Math.max(30, 30 + solarValue / 20))
      : 40;
  ...
  createWindAnimation(card, windSpeed, windDirection) {
    ...
    for (let i = 0; i < lineCount; i++) {
      ...
      const angle =
        Math.random() * 15 - 7.5 + (windDirection ? windDirection - 90 : 0);

      const lineOpacity = Math.min(
        0.7,
        0.1 + windSpeed * 0.02 + Math.random() * 0.1,
      );
  ...
  initTimeDisplays() {
    const updateTimes = () => {
      ...
      const leonardoTime = new Date(
        now.toLocaleString("en-US", { timeZone: "America/New_York" }),
      );
      document.getElementById("leonardo-time").textContent =
        leonardoTime.toLocaleTimeString("en-US", {
          hour: "numeric",
          minute: "2-digit",
          second: "2-digit",
          hour12: true,
      ...
      const florenceTime = new Date(
        now.toLocaleString("en-US", { timeZone: "Europe/Rome" }),
      );
      document.getElementById("florence-time").textContent =
        florenceTime.toLocaleTimeString("en-US", {
          hour: "numeric",
          minute: "2-digit",
          second: "2-digit",
          hour12: false,
  ...
  updateSaunaUI(data) {
    try {
      ...
      let additionalInfoElement = document.getElementById(
        "sauna-additional-info",
      );
      ...
      if (data && data.hasOwnProperty("temperature")) {
        temperatureInFahrenheit = Math.round(
      ...
      if (data && data.hasOwnProperty("targetTemperature")) {
        targetTempFahrenheit = Math.round(
          (parseFloat(data.targetTemperature) * 9) / 5 + 32,
        );
      }
      ...
      if (data && data.hasOwnProperty("statusCode")) {
          ...
          case 231: // Online and heating
            ...
            if (data.targetTemperature) {
              ...
              const progressPercent = Math.min(
                ...
                Math.max(
                  0,
                  Math.round((tempProgress / totalTempJourney) * 100),
                ),
              );
        ...
        saunaTempElement.classList.toggle(
          "sauna-off",
          !isHeating && !isOffline && !isLocked && !isEmergencyStop,
        );
        saunaStatusElement.classList.toggle(
          "sauna-off",
          !isHeating && !isOffline && !isLocked && !isEmergencyStop,
        );
